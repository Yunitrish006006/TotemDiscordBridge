...
public final class DiscordTransientBridge {
    ...
    public static void sendServerStatus(
            ...
            double tps,
            boolean immediately) {
        if (!DiscordTransportService.isEnabled()) {
            return;
        }
    ...
    private static void post(String path, JsonObject payload, String event) {
        String workerUrl = DiscordTransportService.getWorkerUrl();
        String apiKey = readApiKey();
        if (workerUrl == null || workerUrl.isBlank() || apiKey.isBlank()) {
            LOGGER.warn("[DiscordBridge] 無法送出暫時通知 {}：Worker URL 或 API Key 缺失", event);
            return;
        }
    ...
    private static JsonArray channels() {
        ...
        for (DiscordTransportService.DiscordChannel channel : DiscordTransportService.getChannels()) {
    ...
    private static String readApiKey() {
        try {
            Field field = DiscordTransportService.class.getDeclaredField("apiKey");
            ...
            LOGGER.error("[DiscordBridge] 無法讀取 API Key 供暫時通知使用", exception);
            return "";
        }
    }
}
